...
```{r}
cox <- coxph(formula = Surv(futime, fustat) ~ age_factor  + rx + resid.ds, data = df_new)
summary(cox)
...
```{r, message=FALSE}
aa_fit <- aareg(formula = Surv(futime, fustat) ~ age_factor + rx + resid.ds, data = df_new)
autoplot(aa_fit)
```
...
```{r}
fit.coxph <- coxph(Surv(futime, fustat) ~ age_factor + rx + resid.ds, data = df_new)
